...
export default function PlacesScreen() {
  ...
  const handleSubmit = () => {
    ...
    fetch(`https://api-adresse.data.gouv.fr/search/?q=${city}`)
      ...
      .then((data) => {
        ...
        fetch("https://locapic-backend-omega-eosin.vercel.app/places", {
          method: "POST",
          ...
          .then((data) => {
            //console.log(data)
            if (data.result) {
              setSavedPlace([...savedPlace, newPlace]);
              setCity("");
            }
          });
      });
  ...
  useEffect(() => {
    fetch(
      `https://locapic-backend-omega-eosin.vercel.app/places/${user.nickname}`
    )
      .then((response) => response.json())
      .then((data) => {
        //console.log(data)
  ...
  const places = savedPlace.map((data, i) => {
    return (
